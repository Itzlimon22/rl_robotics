...
\begin{figure}[!t]
  \centering
  % Replace with: \includegraphics[width=\columnwidth]{figures/fig2_training_curves.pdf}
  \framebox{\parbox{\columnwidth}{\centering\vspace{1.8cm}
    \textbf{[FIG: training curves]}\\[4pt]
    Goal distance and success rate vs.\ training steps (3 seeds, mean $\pm$ std).\\
    \texttt{python scripts/plot\_training\_curves.py}
    \vspace{1.8cm}}}
  \caption{%
    Training convergence for all three conditions (mean~$\pm$~std, $3$~seeds).
    All conditions converge to $>95\%$ training success.
    \CDR shows slower initial progress (curriculum gated at narrow ranges)
    before accelerating.%
  }
...
\begin{figure*}[!t]
  \centering
  % Replace with: \includegraphics[width=\textwidth]{figures/fig4_results_bar.pdf}
  \framebox{\parbox{\textwidth}{\centering\vspace{2.2cm}
    \textbf{[FIG: results bar chart]}\\[6pt]
    (a) Transfer success rate \quad
    (b) Mean episode reward \quad
    (c) Energy per step\\[4pt]
    Individual seed dots overlaid. Dashed lines: \PID reference.\\
    \texttt{python scripts/plot\_results.py}
    \vspace{2.2cm}}}
  \caption{%
...
\begin{figure}[!t]
  \centering
  % Replace with: \includegraphics[width=\columnwidth]{figures/fig3_curriculum_level.pdf}
  \framebox{\parbox{\columnwidth}{\centering\vspace{1.8cm}
    \textbf{[FIG: curriculum level]}\\[4pt]
    $\lambda$ vs.\ training steps for $3$ seeds.\\
    \texttt{python scripts/plot\_curriculum\_level.py}
    \vspace{1.8cm}}}
  \caption{%
    \CDR curriculum expansion over $10^6$ training steps.
    $\lambda\in[0,1]$ rises from~$0$ (narrow \CDR start ranges) toward~$1$
    (full \UDR training range).
    Brief contractions occur when rolling success drops below~$\tau^-$.
    Seeds reaching higher $\lambda$ achieve higher transfer success.%
  }
