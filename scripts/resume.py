...
from train import (SAC_HYPERPARAMS, EVAL_FREQ, N_EVAL_EPS, CHECKPOINT_FREQ,
                   detect_device, resolve_xml_path,
                   AUVMetricsCallback, CDRCheckpointCallback)
...
def find_run_dir(mode, seed, base_dir=None):
    ...
    for p in candidates:
        if p.exists():
            ...
            return p
    raise FileNotFoundError(f"Run dir not found for mode={mode} seed={seed}. Tried: {candidates}")
...
def find_checkpoint_files(run_dir, checkpoint_step):
    ...
    if not model_path.exists():
        ...
        raise FileNotFoundError(f"model_{checkpoint_step}_steps.zip not found. Available: {available}")

    # Find best vecnormalize at or before checkpoint_step
    vn_files = sorted(ckpt_dir.glob("vecnormalize_*_steps.pkl"))
    if not vn_files:
        raise FileNotFoundError(f"No vecnormalize_*.pkl in {ckpt_dir}")

    vn_steps = [int(p.stem.replace("vecnormalize_","").replace("_steps","")) for p in vn_files]
    valid    = [s for s in vn_steps if s <= checkpoint_step]
    if not valid:
        raise FileNotFoundError(f"No vecnormalize <= {checkpoint_step}. Available: {vn_steps}")

    best_vn   = max(valid)
    vecnorm_path = ckpt_dir / f"vecnormalize_{best_vn}_steps.pkl"
    if best_vn != checkpoint_step:
        print(f"[resume] VecNormalize: using step {best_vn} (closest to {checkpoint_step})")
    return model_path, vecnorm_path


def find_cdr_state(run_dir, checkpoint_step):
    cdr_files = [
        f for f in sorted(run_dir.glob("cdr_state_*.json"))
    ...
    with open(run_dir / f"cdr_state_{best}.json") as f:
        state = json.load(f)
    print(f"[resume] CDR state from step {best}: level={state['curriculum_level']:.3f} "
...
def rebuild_eval_env(xml_path, mode, seed, train_vec):
    ...
    vec = VecNormalize(vec, norm_obs=True, norm_reward=False,
                       clip_obs=10.0, gamma=SAC_HYPERPARAMS["gamma"])
...
def resume(args):
    ...
    train_env = rebuild_train_env(str(xml_path), args.mode, args.seed, vecnorm_path, cdr_state)
    ...
    callbacks = CallbackList([
        AUVMetricsCallback(),
        EvalCallback(eval_env, best_model_save_path=str(run_dir),
        ...
        CheckpointCallback(save_freq=CHECKPOINT_FREQ, save_path=str(ckpt_dir),
                           name_prefix="model", save_vecnormalize=True, verbose=1),
        CDRCheckpointCallback(save_dir=run_dir, save_freq=CHECKPOINT_FREQ, verbose=1),
    ])
    ...
    try:
        ...
        if hasattr(env, "get_cdr_state"):
            ...
            print(f"[save] CDR final: level={state['curriculum_level']:.3f} "
                  f"episodes={state['n_episodes']} successes={state['n_successes']}")
...
def main():
    p = argparse.ArgumentParser(description="Resume interrupted AUV training.",
